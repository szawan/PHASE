...
idTmps = idTmps_csv["sample"].tolist()
print(idTmps)
...
network = SCMIL()
network.load_state_dict(torch.load("./Model_result/PHASE_100.pt"),strict=False)
...
sample_id_order = pd.read_csv("./sampleid_560.csv")["sample"].tolist()

###  sample embedding
attention_matrices = torch.zeros((22, 64), device=device)  # Initialize a tensor to store all attention matrices
